  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Topic: ${dossier.topic}\n\nResearch briefings to draw the conversation from:\n\n${body}\n\nWrite a podcast conversation of about 24-32 exchanges (lines) total that walks through this topic and its subtopics naturally, the way a good explainer podcast would. Open with a hook, close with a takeaway. Each line should be 1-4 sentences, spoken naturally, in your own words.`,
        },
      ],
    });
