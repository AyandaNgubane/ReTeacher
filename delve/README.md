...
- Research is deliberately built as **one Gemini call per dossier**,
  regardless of how many angles you choose: all subtopic briefings and the
  overview come back from a single structured-JSON request, rather than one
  call per subtopic. Search (Tavily/yt-dlp/Open Library) still runs fully in
  parallel per angle, but search isn't rate-limited or Gemini-billed, so
  that doesn't cost extra calls. A full session — subtopics, research,
  optional summary, optional podcast — is about 2-4 Gemini calls total, no
  matter how many angles a dossier covers. `gemini-2.0-flash` keeps this
  both fast and inexpensive; check current pricing at
...
- The tradeoff of one big call: it needs more output headroom
  (`maxOutputTokens: 8192` on the research and podcast routes) and asks the
  model to hold structure across a longer response. If a dossier ever comes
  back with a subtopic missing its briefing, that's `app/api/research/route.ts`
  falling back gracefully after failing to find a matching `subtopicId` in
  Gemini's response — regenerating the dossier usually resolves it.
...
`generateJSON` call queues through the same limiter. With research now
collapsed into a single call, a single dossier session won't come close to
this on its own; it mainly exists as a safety net once more than a couple
of people are using the app at the same time.
...
- All routes run with `maxDuration: 60`, the Vercel Hobby plan's cap —
  intentionally, since with one Gemini call per dossier there's no longer a
  good reason to need more. If you're on Hobby and still see 504s, that
  points at something else being slow (a large number of search results, a
  particularly complex topic) rather than the throttle — check your
  function logs in the Vercel dashboard for the actual error before
  reaching for a Pro plan.
